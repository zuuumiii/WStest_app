...
class WeatherScrapingController < ApplicationController
  ...
  def index
    ...
    @term = (@y_date - @start_date).to_i  #期間＝取得するデータの数
    ...
    @term_month.times do |m|
      ...
      url = "http://www.data.jma.go.jp/obd/stats/etrn/view/daily_s1.php?prec_no=48&block_no=47618&year=#{start_year}&month=#{start_month + m}&day=1&view="
      agent = Mechanize.new
      page = agent.get(url)
